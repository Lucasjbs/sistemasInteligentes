...
public class Modelo {
    ...
    public Modelo(double[][] X_treino, double[] Y_treino, double[][] X_teste, double[] Y_teste) {
        ...
        for (double[] row: W1){
            Arrays.fill(row, Math.random()*5);
        }
        Arrays.fill(W2, Math.random()*5);
        Arrays.fill(b1, Math.random()*5);
        b2 = Math.random()*5;
    }
...
//        }
    ...
    public void treina(double taxa_aprendizado, double num_iteracoes, int tipo){
        ...
        System.out.println("Custo" + " % na " + abc +  " interação: " + custo);
        series.add(custo, abc);
...
//GRAFICO
//        if(abc%10 == 0){
//            double[] Y_predicao_treino = predicao(X_treino,tipo);
//            System.out.println("Acuracia de treino " + (100 - calculaMAE(Y_predicao_treino, Y_treino) * 100) + "% na " + abc +  " interação");
//        }
